...
def draw_polygons( polygons, screen, color ):
	for i in range(0,len(polygons),3):
		...
		draw_line(int(p2[0]),int(p2[1]),int(p3[0]),int(p3[1]),screen,color)
		draw_line(int(p3[0]),int(p3[1]),int(p1[0]),int(p1[1]),screen,color)
		...
		for longt in range(longt_start, longt_stop):
			...
			if lat == lat_stop -1:
				...
				if longt == 0:
					p0 = points[index]
					p1 = points[index+1]
					p2 = points[index+step+1]
				else:
					p0 = points[index]
					p1 = points[index+1]
					p2 = points[index+step]
